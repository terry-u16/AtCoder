...
namespace AtCoderBeginnerContest129.Questions
{
    public class QuestionC : AtCoderQuestionBase
    {
        public override IEnumerable<object> Solve(TextReader inputStream)
        {
            ...
            var stairCount = nm[0];
            var brokenStairCount = nm[1];
            var brokenStairs = new HashSet<int>();

            for (int i = 0; i < brokenStairCount; i++)
            {
                brokenStairs.Add(inputStream.ReadInt());
            }

            var count = new Modular[stairCount + 1];
            for (int i = 0; i < count.Length; i++)
            {
                count[i] = new Modular(0);
            }

            count[0] = new Modular(1);

            for (int stair = 1; stair <= stairCount; stair++)
            {
                if (!brokenStairs.Contains(stair))
                {
                    count[stair] += count[stair - 1];

                    if (stair > 1)
                    {
                        count[stair] += count[stair - 2];
                    }
                }
            }

            yield return count[stairCount].Value;
        }
