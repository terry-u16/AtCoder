...
namespace AtCoderBeginnerContest151.Questions
{
    ...
    public class QuestionE_Review : AtCoderQuestionBase
    {
        public override IEnumerable<object> Solve(TextReader inputStream)
        {
            ...
            var maxSum = new Modular(0);
            var minSum = new Modular(0);

            for (int i = k; i <= n; i++)
            {
                var sub = i - 1;
                var combination = sub > 0 ? Modular.Combination(sub, k - 1) : new Modular(1);
                maxSum += combination * new Modular(a[i - 1]);
            }
            ...
            for (int i = 1; i <= n - k + 1; i++)
            {
                var sub = n - i;
                var combination = sub > 0 ? Modular.Combination(sub, k - 1) : new Modular(1);
                minSum += combination * new Modular(a[i - 1]);
            }

            yield return (maxSum - minSum).Value;
        }
    }
