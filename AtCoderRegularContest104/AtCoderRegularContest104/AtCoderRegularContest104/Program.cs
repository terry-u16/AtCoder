...
namespace AtCoderRegularContest104
{
    class Program
    {
...
namespace AtCoderRegularContest104.Numerics
{
...
namespace AtCoderRegularContest104.Algorithms
{
...
namespace AtCoderRegularContest104.Collections
{
...
namespace AtCoderRegularContest104.Graphs
{
...
namespace AtCoder.Internal
{
    public static class Butterfly<T> where T : struct, IStaticMod
    {
        ...
        public static void Calculate(Span<StaticModInt<T>> a)
        {
            ...
            if (Avx2.IsSupported && a.Length >= Vector256<uint>.Count * 2)
            {
                CalculateVector(a);
            }
            else
        ...
        private static void CalculateScalar(Span<StaticModInt<T>> a)
        {
            var n = a.Length;
        ...
        [MethodImpl(MethodImplOptions.AggressiveOptimization)]
        private static unsafe void CalculateVector(Span<StaticModInt<T>> a)
        {
            var n = a.Length;
            var h = InternalMath.CeilPow2(n);

            // 全要素がmodのVector<uint>を作成（比較および加減算用）
            var mods = Vector256.Create(default(T).Mod);
            ...
            for (int ph = 1; ph <= h; ph++)
            {
                ...
                AddAndSubtractAll(a, h, mods, ph, w, p);
            }
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private static (Vector256<uint> add, Vector256<uint> sub) AddAndSubtract(Vector256<uint> u, Vector256<uint> v, Vector256<uint> mods)
        {
            var add = Avx2.Add(u, v);
            var sub = Avx2.Subtract(u, v);
            var addMod = Avx2.Subtract(add, mods);
            var geMask = GreaterEqual(add.AsInt32(), mods.AsInt32()).AsUInt32();
            add = Avx2.BlendVariable(add, addMod, geMask);

            var subMod = Avx2.Add(sub, mods);
            var ltMask = LessThan(sub.AsInt32(), Vector256<int>.Zero).AsUInt32();
            sub = Avx2.BlendVariable(sub, subMod, ltMask);
            return (add, sub);
        }
        ...
        public static void CalculateInv(Span<StaticModInt<T>> a)
        {
            ...
            if (Avx2.IsSupported && a.Length >= Vector256<uint>.Count * 2)
            {
                CalculateVectorInv(a);
            }
            else
            {
                CalculateScalarInv(a);
            }
        }

        [MethodImpl(MethodImplOptions.AggressiveOptimization)]
        public static void CalculateScalarInv(Span<StaticModInt<T>> a)
        {
            var n = a.Length;
            var h = InternalMath.CeilPow2(n);
        ...
        [MethodImpl(MethodImplOptions.AggressiveOptimization)]
        public static unsafe void CalculateVectorInv(Span<StaticModInt<T>> a)
        {
            var n = a.Length;
            var h = InternalMath.CeilPow2(n);
        ...
        private static unsafe void AddAndSubtractAll(Span<StaticModInt<T>> a, int h, Vector256<uint> mods, int ph, int w, int p)
        {
            fixed (uint* baseAddress = MemoryMarshal.Cast<StaticModInt<T>, uint>(a))
            {
                switch (p)
                {
                    case 1:
                        AddAndSubtract1(mods, baseAddress, a.Length);
                        break;
                    case 2:
                        AddAndSubtract2(mods, baseAddress, a.Length);
                        break;
                    case 4:
                        AddAndSubtract4(mods, baseAddress, a.Length);
                        break;
                    default:
                        for (int s = 0; s < w; s++)
                        {
                            var ls = s << (h - ph + 1);
                            var rs = ls + p;
                            AddAndSubtractEach(mods, p, baseAddress + ls, baseAddress + rs);
                        }
                        break;
                }
            }
        ...
        static unsafe void AddAndSubtract1(Vector256<uint> mods, uint* baseAddress, int length)
        {
            for (int offset = 0; offset < length; offset += Vector256<uint>.Count * 2)
            {
                var lsp = baseAddress + offset;
                var rsp = lsp + Vector256<uint>.Count;

                var u = Avx.LoadVector256(lsp);
                var v = Avx.LoadVector256(rsp);
                (u, v) = Shuffle1(u, v);

                var (add, sub) = AddAndSubtract(u, v, mods);
                (add, sub) = Shuffle1Inv(add, sub);

                Avx2.MaskStore(lsp, Vector256.Create(uint.MaxValue), add);
                Avx2.MaskStore(rsp, Vector256.Create(uint.MaxValue), sub);
            }
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
        static unsafe void AddAndSubtract2(Vector256<uint> mods, uint* baseAddress, int length)
        {
            for (int offset = 0; offset < length; offset += Vector256<uint>.Count * 2)
            {
                var lsp = baseAddress + offset;
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
        static unsafe void AddAndSubtract4(Vector256<uint> mods, uint* baseAddress, int length)
        {
            for (int offset = 0; offset < length; offset += Vector256<uint>.Count * 2)
            {
                var lsp = baseAddress + offset;
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
        static unsafe void AddAndSubtractEach(Vector256<uint> mods, int p, uint* baseAddressL, uint* baseAddressR)
        {
            for (int pOffset = 0; pOffset < p; pOffset += Vector256<uint>.Count)
            {
                var lspCurrent = baseAddressL + pOffset;
                var rspCurrent = baseAddressR + pOffset;

                var u = Avx.LoadVector256(lspCurrent);
                var v = Avx.LoadVector256(rspCurrent);

                var (add, sub) = AddAndSubtract(u, v, mods);

                Avx2.MaskStore(lspCurrent, Vector256.Create(uint.MaxValue), add);
                Avx2.MaskStore(rspCurrent, Vector256.Create(uint.MaxValue), sub);
            }
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
        private static (Vector256<uint> l, Vector256<uint> r) Shuffle1(Vector256<uint> v1, Vector256<uint> v2)
        {
            var s1 = Avx2.Shuffle(v1, 0b10_00_10_00);
            var s2 = Avx2.Shuffle(v2, 0b10_00_10_00);
            var l = Avx2.Blend(s1, s2, 0b11_00_11_00);
            s1 = Avx2.Shuffle(v1, 0b11_01_11_01);
            s2 = Avx2.Shuffle(v2, 0b11_01_11_01);
            var r = Avx2.Blend(s1, s2, 0b11_00_11_00);
            return (l, r);
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
        private static (Vector256<uint> v1, Vector256<uint> v2) Shuffle1Inv(Vector256<uint> l, Vector256<uint> r)
        {
            var s1 = Avx2.Shuffle(l, 0b01_01_00_00);
            var s2 = Avx2.Shuffle(r, 0b01_01_00_00);
            var v1 = Avx2.Blend(s1, s2, 0b10_10_10_10);
            s1 = Avx2.Shuffle(l, 0b11_11_10_10);
            s2 = Avx2.Shuffle(r, 0b11_11_10_10);
            var v2 = Avx2.Blend(s1, s2, 0b10_10_10_10);
            return (v1, v2);
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
        private static (Vector256<uint> l, Vector256<uint> r) Shuffle2(Vector256<uint> v1, Vector256<uint> v2)
        {
            var s1 = Avx2.Shuffle(v1, 0b01_00_01_00);
            var s2 = Avx2.Shuffle(v2, 0b01_00_01_00);
            var l = Avx2.Blend(s1, s2, 0b11_00_11_00);
            s1 = Avx2.Shuffle(v1, 0b11_10_11_10);
            s2 = Avx2.Shuffle(v2, 0b11_10_11_10);
            var r = Avx2.Blend(s1, s2, 0b11_00_11_00);
            return (l, r);
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
        private static (Vector256<uint> v1, Vector256<uint> v2) Shuffle2Inv(Vector256<uint> l, Vector256<uint> r)
        {
            var s1 = Avx2.Shuffle(l, 0b01_00_01_00);
            var s2 = Avx2.Shuffle(r, 0b01_00_01_00);
            var v1 = Avx2.Blend(s1, s2, 0b11_00_11_00);
            s1 = Avx2.Shuffle(l, 0b11_10_11_10);
            s2 = Avx2.Shuffle(r, 0b11_10_11_10);
            var v2 = Avx2.Blend(s1, s2, 0b11_00_11_00);
            return (v1, v2);
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
        private static (Vector256<uint> l, Vector256<uint> r) Shuffle4(Vector256<uint> v1, Vector256<uint> v2)
        {
            var r1 = Avx2.ExtractVector128(v1, 1);
            var l2 = Avx2.ExtractVector128(v2, 0);
            var l = Avx2.InsertVector128(v1, l2, 1);
            var r = Avx2.InsertVector128(v2, r1, 0);
            return (l, r);
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
        private static (Vector256<uint> v1, Vector256<uint> v2) Shuffle4Inv(Vector256<uint> l, Vector256<uint> r)
        {
            var l2 = Avx2.ExtractVector128(l, 1);
            var r1 = Avx2.ExtractVector128(r, 0);
            var v1 = Avx2.InsertVector128(l, r1, 1);
            var v2 = Avx2.InsertVector128(r, l2, 0);
            return (v1, v2);
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
        static Vector256<int> GreaterEqual(Vector256<int> lhs, Vector256<int> rhs)
        {
            var eq = Avx2.CompareEqual(lhs, rhs);
            var gt = Avx2.CompareGreaterThan(lhs, rhs);
            return Avx2.Or(eq, gt);
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
        static Vector256<int> LessThan(Vector256<int> lhs, Vector256<int> rhs) => Avx2.CompareGreaterThan(rhs, lhs);
