...
namespace AtCoderLibraryPracticeContest.Questions
{
    public class QuestionD : AtCoderQuestionBase
    {
        public override IEnumerable<object> Solve(TextReader inputStream)
        {
            ...
            var graph = new MFGraphInt(n + 2);

            for (int row = 0; row < height; row++)
            {
                for (int column = 0; column < width; column++)
                {
                    var from = GetIndex(row, column);

                    if (((row + column) & 1) == 0 && map[row][column] != '#')
                    {
                        foreach (var (dr, dc) in diffs)
                        {
                            ...
                            if (unchecked((uint)nextRow < height && (uint)nextColumn < width) && map[nextRow][nextColumn] != '#')
                            {
                                var to = GetIndex(nextRow, nextColumn);
            ...
            var matches = graph.Flow(n, n + 1);

            foreach (var edge in graph.Edges())
            {
                if (edge.Flow == 1 && edge.From < n && edge.To < n)
                {
                    var from = edge.From;
                    var to = edge.To;
                    ...
                    var (fromRow, fromColumn) = GetRowColumn(from);
                    var (toRow, toColumn) = GetRowColumn(to);

                    if (fromRow == toRow)
                    {
                        map[fromRow][fromColumn] = '>';
                        map[toRow][toColumn] = '<';
                    }
                    else
                    {
                        map[fromRow][fromColumn] = 'v';
                        map[toRow][toColumn] = '^';
                    }
                }
            }
            ...
            {
                yield return map[row].Join();
            }

            int GetIndex(int row, int column) => row * width + column;
            (int r, int c) GetRowColumn(int index) => (index / width, index % width);
        }
