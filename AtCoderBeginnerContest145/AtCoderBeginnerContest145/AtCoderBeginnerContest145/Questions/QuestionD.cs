...
namespace AtCoderBeginnerContest145.Questions
{
    public class QuestionD : AtCoderQuestionBase
    {
        public override IEnumerable<object> Solve(TextReader inputStream)
        {
            ...
            var totalJumpCount = (x + y) / 3;
            var rightJumpCount = x - totalJumpCount;

            yield return Modular.Combination(totalJumpCount, rightJumpCount).Value;
        }

        bool IsReachable(int x, int y) => x >= y / 2 && y >= x / 2 && (x + y) % 3 == 0;
    }
